...
- **品牌色**：主蓝 `#1767ff`（深阶 `#0754e5`），浅冰蓝底 `#f4f8ff`，正文 `#12284c`。
- **圆角阶梯**：`--r-xs 6` / `--r-sm 8` / `--r-md 12` / `--r-lg 16` / `--r-pill 999`（px）。
- **景深层级**：`--elev-1`（静置卡片）/ `--elev-2`（悬浮态、面板）/ `--elev-3`（弹窗），均含内描边高光。
- **响应式断点**：`max-width:1100px`（平板）、`max-width:760px`（手机）、`min-width:1450px`（宽屏首屏图改为 contain）。
- **动效**：滚动入场与指标数字滚动均由 `prefers-reduced-motion` 保护，减弱动效时内容完整可见。
